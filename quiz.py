...
class Application(tk.Frame):
    ...
    def create_widget(self):
        ...
        # 得点版
        label = tk.Label(self, relief=tk.RIDGE, width=17, height=3, textvariable=self.score_text, font=("游ゴシック体", "20", "bold"), bg="white")
    ...
    def create_question(self):
        ...
        for i,choices in enumerate(questions[self.question_number]["選択肢"]):
            if i + 1 == questions[self.question_number]["正解"]:
                correct_button = tk.Button(self, width=28, text=choices, font=("游ゴシック体", "20", "bold"), bg="white", command=self.correct)
                correct_button.place(x=10, y=350 + (i * 60))
            else:
                incorrect_button = tk.Button(self, width=28, text=choices, font=("游ゴシック体", "20", "bold"), bg="white", command=self.incorrect)
